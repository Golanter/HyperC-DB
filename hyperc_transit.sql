CREATE OR REPLACE PROCEDURE public.hyperc_transit(IN sql_command character varying, IN plan_id character varying)
 LANGUAGE plpython3u
AS $procedure$
...
if op_time != -1:
    et.metadata.update({"run_rule_optimization": op_time, "ignore_cached": False})
if DUMP_DATA:
    import pickle
